...
@pytest.mark.core_model
@pytest.mark.cpu
def test_load_model_skips_compile_in_batch_invariant_mode(monkeypatch):
    class _Pipeline:
        def __init__(self):
            self.setup_compile_called = False

        def setup_compile(self):
            self.setup_compile_called = True
    ...
    monkeypatch.setattr(model_runner_module, "is_batch_invariant_enabled", lambda: True)
    ...
    monkeypatch.setattr(
        ...
        lambda self, attr_name: pytest.fail(f"unexpected compile for {attr_name}"),
    )

    DiffusionModelRunner.load_model(runner)

    assert pipeline.setup_compile_called is False
