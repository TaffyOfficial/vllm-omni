...
class TestRequestScheduler:
    ...
    def test_batch_invariant_orders_waiting_by_priority_and_arrival_order(
        ...
        scheduler.update_from_output(first, _make_request_output("c"))

        second = scheduler.schedule()
        assert _new_ids(second) == ["a"]
        assert second.num_running_reqs == 1
        assert second.num_waiting_reqs == 1

        scheduler.update_from_output(second, _make_request_output("a"))
    ...
    def test_batch_invariant_uses_arrival_order_before_numeric_request_id(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.setenv("VLLM_BATCH_INVARIANT", "1")
        scheduler = RequestScheduler()
        ...
        for req_id in ["0", "1", "10", "2"]:
            scheduler.add_request(_make_request(req_id, priority=0, seed=1))

        for req_id, waiting in [("0", 3), ("1", 2), ("10", 1), ("2", 0)]:
            output = scheduler.schedule()
            assert _new_ids(output) == [req_id]
            assert output.num_running_reqs == 1
            assert output.num_waiting_reqs == waiting
            scheduler.update_from_output(output, _make_request_output(req_id))
