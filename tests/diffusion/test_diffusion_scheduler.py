...
class TestRequestScheduler:
    ...
    def test_batch_invariant_request_id_tie_break_uses_child_id(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("VLLM_BATCH_INVARIANT", "1")
        scheduler = RequestScheduler()
        scheduler.initialize(SimpleNamespace(max_num_seqs=2))

        scheduler.add_request(_make_request("z", priority=0, seed=1))
        scheduler.add_request(_make_request("a", priority=0, seed=2))

        sched_output = scheduler.schedule()

        assert _new_ids(sched_output) == ["a"]
