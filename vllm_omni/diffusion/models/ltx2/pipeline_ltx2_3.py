...
class _LTX23I2VVideoAudioScheduler:
    """Composite scheduler for LTX-2.3 I2V.

    The video branch keeps the first conditioning frame latent fixed while the
    audio branch follows the regular scheduler step.
    """

    def __init__(
        self,
        pipeline: LTX23ImageToVideoPipeline,
...
class LTX23ImageToVideoPipeline(LTX23Pipeline):
    ...
    def forward(
        ...
        audio_scheduler = copy.deepcopy(self.scheduler)
        video_audio_scheduler = _LTX23I2VVideoAudioScheduler(
            self,
            audio_scheduler,
            latent_num_frames,
            latent_height,
