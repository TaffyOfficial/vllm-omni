...
class DiffusionModelRunner(OmniConnectorModelRunnerMixin):
    ...
    def load_model(
        ...
        # Apply torch.compile if not in eager mode. Batch-invariant execution
        # must avoid batch-shape-dependent compiled graph paths.
        if is_batch_invariant_enabled():
            logger.info("Model runner: skipping torch.compile because VLLM_BATCH_INVARIANT is enabled.")
        elif not self.od_config.enforce_eager:
